...
class TestIsVisualInput:
    def test_png_is_visual(self, tmp_path):
        ...
        assert FileProcessorRefactored._is_visual_input(f) is True
    ...
    def test_txt_is_not_visual(self, tmp_path):
        f = tmp_path / "file.txt"
        f.write_text("hello")
        assert FileProcessorRefactored._is_visual_input(f) is False

    def test_directory_with_images_is_visual(self, tmp_path):
        (tmp_path / "a.png").write_bytes(b"x")
        assert FileProcessorRefactored._is_visual_input(tmp_path) is True

    def test_directory_with_only_text_is_not_visual(self, tmp_path):
        (tmp_path / "a.txt").write_text("text")
        assert FileProcessorRefactored._is_visual_input(tmp_path) is False
    ...
    def test_nonexistent_path(self, tmp_path):
        ...
        assert FileProcessorRefactored._is_visual_input(fake) is False

...
class TestCM2ServiceTierBatchMode:
    ...
    def test_batch_strategy_stores_concurrency_config(self):
        ...
        from modules.extract.processing_strategy import BatchProcessingStrategy
...
class TestCM9ResumeBackNavigation:
    ...
    Uses AST inspection rather than substring matching so the test is robust
    to whitespace and comment changes. Invariants enforced:
    - Somewhere inside ``_run_interactive_mode`` a ``select_option(...)``
      call is made with the ``allow_back=True`` keyword.
    - Somewhere in that function the back branch assigns
      ``current_step = "batch"``.
    """
    ...
    def _source_ast(obj):
        ...
        return ast.parse(inspect.getsource(obj).lstrip())
    ...
    def test_resume_step_uses_select_option_with_allow_back(self):
        ...
        for node in ast.walk(tree):
            ...
            if found:
                break
        assert found, "resume step must call select_option(..., allow_back=True)"
...
class TestCM11GatherReturnExceptions:
    ...
    AST inspection is used so the test is robust to whitespace/comment
    changes; any ``asyncio.gather(...)`` call inside the target function
    must carry ``return_exceptions=True``.
    """

    @staticmethod
    def _gather_has_return_exceptions_true(func_obj) -> bool:
        import ast
        import inspect

        tree = ast.parse(inspect.getsource(func_obj).lstrip())
        for node in ast.walk(tree):
            if not isinstance(node, ast.Call):
                continue
            func = node.func
    ...
    def test_exceptions_from_gather_are_logged(self):
        """The gather caller filters Exception items from the results list.

        With ``return_exceptions=True`` the gather result may contain
        Exception instances. The module must type-check entries via
        ``isinstance(..., Exception)`` so failures are handled rather than
        silently swallowed.
        """
        import ast
        import inspect

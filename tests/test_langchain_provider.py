...
class TestProviderConfig:
    ...
    def test_provider_config_with_reasoning_config(self):
        ...
        # Regression: the dataclass field must also be set, not left None.
    ...
    def test_provider_config_provider_override(self):
        ...
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "test-key"}):
            config = ProviderConfig.from_config(
                model_config, provider_override="anthropic"
            )
...
class TestNormalizeSchemaForAnthropic:
    ...
    def test_normalize_list_schema(self):
        ...
        schema = [{"type": "string"}, {"type": ["integer", "null"]}]
        result = _normalize_schema_for_anthropic(schema)
...
class TestBuildReasoningPayload:
    ...
    def test_with_effort(self):
        ...
        result = _build_reasoning_payload("gpt-4o", {"effort": "high"}, 10000)
    ...
    def test_with_explicit_max_tokens(self):
        ...
        result = _build_reasoning_payload("gpt-4o", {"max_tokens": 5000}, 10000)
        assert result is not None
    ...
    def test_with_exclude_flag(self):
        ...
        result = _build_reasoning_payload("gpt-4o", {"exclude": True}, 10000)
        assert result is not None
    ...
    def test_with_enabled_flag(self):
        ...
        result = _build_reasoning_payload("gpt-4o", {"enabled": False}, 10000)
        assert result is not None
    ...
    def test_anthropic_model_converts_effort_to_max_tokens(self):
        ...
        result = _build_reasoning_payload(
            "anthropic/claude-3.5-sonnet", {"effort": "high"}, 10000
        )
        assert result is not None
    ...
    def test_deepseek_model_converts_effort_to_enabled(self):
        ...
        result = _build_reasoning_payload(
            "deepseek/deepseek-chat", {"effort": "high"}, 10000
        )
        assert result is not None
        assert "effort" not in result
    ...
    def test_deepseek_none_effort_disables(self):
        ...
        result = _build_reasoning_payload(
            "deepseek/deepseek-chat", {"effort": "none"}, 10000
        )
        assert result is not None
        assert result["enabled"] is False
...
class TestCustomProviderRouting:
    """Test that custom provider is properly handled."""

    @pytest.mark.unit
    def test_custom_provider_supports_structured_outputs(self):
        """Custom provider should report supports_structured_outputs=True."""
    ...
    @pytest.mark.unit
    def test_custom_provider_capabilities_provider_field(self):
        """Custom provider capabilities should report provider='custom'."""
        config = ProviderConfig(
            provider="custom",
            model="rednote-hilab/dots.mocr",
            api_key="test-key",
        ...
        llm = LangChainLLM(config)
        caps = llm._get_capabilities()
        assert caps.provider == "custom"

    @pytest.mark.unit
    def test_invoke_with_schema_has_custom_branch(self):
        """_invoke_with_schema should route custom to _invoke_openai_structured."""
        config = ProviderConfig(
            provider="custom",
            model="org/model",
            api_key="test-key",
        ...
        llm = LangChainLLM(config)
        # Patch the OpenAI structured method to verify it gets called
        llm._invoke_openai_structured = AsyncMock(return_value="ok")

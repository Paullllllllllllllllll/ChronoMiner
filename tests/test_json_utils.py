...
def test_extract_entries_direct_format(tmp_path):
    ...
    json_file.write_text(
        json.dumps(
            {"entries": [{"id": 1, "name": "entry1"}, {"id": 2, "name": "entry2"}]}
        ),
        encoding="utf-8",
    )
...
def test_extract_entries_with_no_content_flag(tmp_path):
    ...
    json_file.write_text(
        json.dumps({"contains_no_content_of_requested_type": True, "entries": []}),
        encoding="utf-8",
    )
...
def test_extract_entries_filters_none_values(tmp_path):
    ...
    json_file.write_text(
        json.dumps({"entries": [{"id": 1}, None, {"id": 2}, None]}), encoding="utf-8"
    )
                ...
                "records": [
                    {
                        ...
                        "response": {
                            "choices": [
                                {
                                    "message": {
                                        "content": json.dumps({"entries": [{"id": 1}]})
                                    }
                                }
                            ]
                        },
                    ...
                    },
                    {
                        ...
                        "response": {
                            "choices": [
                                {
                                    "message": {
                                        "content": json.dumps({"entries": [{"id": 2}]})
                                    }
                                }
                            ]
                        },
        ...
        json.dumps(
            {
                "records": [
                    ...
                    {"custom_id": "chunk-2", "response": {"entries": [{"id": 5}]}},
                ]
            }
        ),
        encoding="utf-8",
        ...
        json.dumps(
            {
                "records": [
                    {
                        "custom_id": "c-1",
                        "response": {"entries": [{"id": 1}, None, {"id": 2}]},
                    }
                ]
            }
        ),
...
def test_extract_json_pure_json():
    """Pure JSON passes through unchanged (fast path)."""
    text = '{"entries": [{"id": 1}]}'
    assert _extract_json_from_text(text) == text


@pytest.mark.unit
def test_extract_json_pure_json_with_whitespace():
    """Whitespace-wrapped JSON is stripped and returned."""
    text = '  \n {"entries": []}  \n'
    assert _extract_json_from_text(text) == '{"entries": []}'


@pytest.mark.unit
def test_extract_json_markdown_fenced():
    """JSON inside ```json code fence is extracted."""
...
def test_extract_json_bare_fence():
    """JSON inside bare ``` fence (no language tag) is extracted."""
    text = '```\n{"entries": [{"id": 2}]}\n```'
    result = _extract_json_from_text(text)
    assert result is not None
    parsed = json.loads(result)
    assert parsed["entries"] == [{"id": 2}]


@pytest.mark.unit
def test_extract_json_preamble_text():
    """JSON with preamble text is found via brace scanning."""
    text = 'Here is the extracted data: {"entries": [{"id": 3}]}'
    result = _extract_json_from_text(text)
    assert result is not None
    parsed = json.loads(result)
    assert parsed["entries"] == [{"id": 3}]


@pytest.mark.unit
def test_extract_json_nested_braces():
    """Nested braces inside strings are handled correctly."""
    obj = {"entries": [{"text": 'a {nested} value "with quotes"'}]}
    text = f"Result: {json.dumps(obj)}"
    result = _extract_json_from_text(text)
    assert result is not None
    parsed = json.loads(result)
    assert parsed["entries"][0]["text"] == 'a {nested} value "with quotes"'
...
@pytest.mark.unit
def test_parse_entries_markdown_fenced():
    """_parse_entries_from_text handles markdown-fenced JSON."""
    text = '```json\n{"entries": [{"id": 10}]}\n```'
    result = _parse_entries_from_text(text)
    assert result == [{"id": 10}]
...
@pytest.mark.unit
def test_parse_entries_with_preamble():
    """_parse_entries_from_text handles JSON with preamble."""
    text = 'The extracted entries are: {"entries": [{"id": 20}]}'
    result = _parse_entries_from_text(text)
    assert result == [{"id": 20}]


@pytest.mark.unit
def test_parse_entries_pure_json_still_works():
    """Pure JSON continues to work after the fallback was added."""
    text = json.dumps({"entries": [{"id": 1}, {"id": 2}]})
    result = _parse_entries_from_text(text)

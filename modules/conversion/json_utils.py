...
def _extract_json_from_text(text: str) -> str | None:
    ...
    code fences or other wrapper content.

    Tries in order:

    1. Direct — text already starts with ``{`` or ``[`` (fast path)
    2. Fenced — extract from ````json ... ```` or bare ```` ... ````
    3. Balanced-brace scan — find first ``{ ... }`` respecting strings

    :param text: Raw model output
    :return: Extracted JSON string, or *None* if nothing found
    """
    stripped = text.strip()
    if not stripped:
        return None

    # 1. Fast path: already looks like JSON
    if stripped[0] in ("{", "["):
        return stripped
...
def _parse_entries_from_text(text: str) -> list[Any] | None:
    ...
    Uses :func:`_extract_json_from_text` to handle markdown-wrapped or
    preamble-prefixed responses from models that do not enforce strict
    structured output.
    ...
    if json_str is None:
        logger.warning("Could not extract JSON from response: %s...", text[:200])
        return None

    try:
        parsed = json.loads(json_str)
    except json.JSONDecodeError:
        logger.warning("Failed to parse extracted JSON: %s...", json_str[:200])
        return None
...
# ---------------------------------------------------------------------------
# Public aliases for helpers that other modules need to call. The underscored
# names remain as the implementation; callers outside this module should use
# these public spellings.
# ---------------------------------------------------------------------------

parse_llm_response_text = _extract_text_from_api_body
parse_json_from_text = _extract_json_from_text

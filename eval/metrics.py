...
def normalize_string(
    text: str, lowercase: bool = True, normalize_ws: bool = True
) -> str:
    """
...
def compare_values(
    ...
    if isinstance(gt_value, list) and isinstance(hyp_value, list):
        return compare_lists(
            gt_value, hyp_value, threshold, case_sensitive, normalize_ws
        )
    ...
    for gt_item in gt_list:
        ...
        for i, hyp_item in enumerate(hyp_list):
            ...
            _, score = compare_values(
                gt_item, hyp_item, threshold, case_sensitive, normalize_ws
            )
            if score > best_score:
                best_score = score
                best_idx = i
...
def compare_dicts(
    ...
    for key in all_keys:
        ...
        matched, score = compare_values(
            gt_val, hyp_val, threshold, case_sensitive, normalize_ws
        )
        total_score += score
...
def compute_extraction_metrics(
    ...
    if key_fields is None:
        key_fields = (
            fields_to_evaluate[:2]
            if len(fields_to_evaluate) >= 2
            else fields_to_evaluate
        )
    ...
    for gt_idx, hyp_idx, match_score in entry_matches:
        ...
        entry_metric = EntryMetrics(
            entry_index=gt_idx, matched=True, overall_score=match_score
        )
    ...
    for gt_idx, gt_entry in enumerate(gt_entries):
        if gt_idx not in matched_gt_indices:
            entry_metric = EntryMetrics(
                entry_index=gt_idx, matched=False, overall_score=0.0
            )
...
if __name__ == "__main__":
    ...
    hypothesis = {
        "entries": [
            ...
            {
                "title": "Mastering Art of French Cooking",
                "author": "Julia Child",
                "year": 1962,
            },
        ]

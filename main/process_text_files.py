...
def validate_absolute_paths(paths_config: Dict[str, Any]) -> None:
	...
	if logs_dir and not os.path.isabs(logs_dir):
		print(
			f"Error: The 'logs_dir' path '{logs_dir}' is not absolute. Please use an absolute path.")
		error_found = True
	...
	for schema, schema_config in schemas_paths.items():
		...
		if input_path and not os.path.isabs(input_path):
			print(
				f"Error: The input path for schema '{schema}' ('{input_path}') is not absolute. Please use an absolute path.")
			error_found = True
		if output_path and not os.path.isabs(output_path):
			print(
				f"Error: The output path for schema '{schema}' ('{output_path}') is not absolute. Please use an absolute path.")
			error_found = True
...
async def process_file(
	...
	if manual_adjust:
		...
		console_print(
			"2. Automatic token-based chunking with manual re-adjustments")
		...
		if strategy_choice == "1":
			...
			line_ranges_file = file_path.with_name(
				file_path.stem + "_line_ranges.txt")
		else:
			console_print(
		...
		default_line_ranges_file: Path = file_path.with_name(
			file_path.stem + "_line_ranges.txt")
		if default_line_ranges_file.exists():
			chunk_choice = "line_ranges.txt"
			line_ranges_file = default_line_ranges_file
			console_print(
				f"Using existing _line_ranges.txt for file {file_path.name}.")
			logger.info(f"Using _line_ranges.txt file for {file_path}")
		else:
			chunk_choice = "auto"
			line_ranges_file = None
			console_print(
				f"Using automatic token-based chunking for file {file_path.name}.")
	...
	if use_batch:
		...
		if not keep_temp:
			...
			logger.info(
				"Non-batch processing, but retain_temporary_jsonl is true. Keeping temporary JSONL.")
...
async def main() -> None:
	...
	# Validate that all critical paths are absolute
	...
	chunking_config_path: Path = Path(
		__file__).resolve().parent.parent / "config" / "chunking_config.yaml"
	...
	try:
		...
		selected_schema: Dict[str, Any] = available_schemas[
			selected_schema_name]
		selected_dev_message: Optional[str] = schema_manager.get_dev_message(
			selected_schema_name)
	except (ValueError, IndexError):
		console_print("Invalid schema selection.")
		sys.exit(1)
	schemas_paths: Dict[str, Any] = config_loader.get_paths_config()[
		"schemas_paths"]
	if selected_schema_name not in schemas_paths:
		console_print(
			f"Schema {selected_schema_name} not found in schemas_paths.")
		sys.exit(1)
	...
	if mode == "1":
		...
		file_candidates = [f for f in file_candidates if
		                   not f.name.endswith("_line_ranges.txt")]
		...
		files_to_process = [f for f in raw_text_dir.rglob("*.txt") if
		                    not f.name.endswith("_line_ranges.txt")]
	...
	for file_path in files_to_process:
		await process_file(
			...
			use_batch=(input(
				"Do you want to use batch processing? (y/n): ").strip().lower() == "y"),

...
async def _adjust_line_ranges_workflow(
    ...
    """
    context_window = chunking_config.get("chunking", {}).get(
        "line_range_context_window", 6
    )
    ...
    for text_file in files:
        ...
        if not line_ranges_file.exists():
            ...
            logger.info(
                f"Line ranges file not found for {text_file.name}, generating..."
            )

            try:
                # Get model configuration
                model_name = model_config.get("extraction_model", {}).get(
                    "name", "o3-mini"
                )
                tokens_per_chunk = chunking_config.get("chunking", {}).get(
...
async def _run_interactive_mode(
        ...
        if current_step == "schema":
            ...
            if state["selected_schema_name"] in schemas_paths:
                schema_input_dir = Path(
                    schemas_paths[state["selected_schema_name"]].get("input", "")
                )
            ...
            resume_choice = ui.select_option(
                ...
                [
                    ...
                    (
                        "reprocess",
                        "Reprocess everything - process all files from scratch",
                    ),
                ],
                allow_back=True,
            )
    ...
    inject_schema = model_config.get("extraction_model", {}).get(
...
async def _run_cli_mode(
    ...
    if (
        ...
        and "gpt-5" not in str(effective_model_name).lower()
    ):
        logger.warning(
    ...
    # Process files sequentially if token limiting is enabled (for better control)
    # Otherwise process concurrently for speed
    inject_schema = effective_model_config.get("extraction_model", {}).get(
        "inject_schema_into_prompt", True
    )
    token_limit_enabled = check_token_limit_enabled()

    # Build context_override from CLI --context flag
    context_override = None
    if getattr(args, "context", None) is not None:
        ctx_val = args.context
        if ctx_val == "auto":
            context_override = {"mode": "auto", "path": None}
        elif ctx_val == "none":
            context_override = {"mode": "none", "path": None}
        else:
            ctx_path = resolve_path(ctx_val)
            if not ctx_path.exists():
                logger.error(f"Context file not found: {ctx_path}")
                print(f"[ERROR] Context file not found: {ctx_path}")
                sys.exit(1)
            context_override = {"mode": "manual", "path": str(ctx_path)}

...
class CancelBatchesScript(DualModeScript):
    ...
    def _load_root_folders(self) -> None:
        ...
        try:
            ...
            input_paths_is_output_path = general.get(
    ...
    def run_interactive(self) -> None:
        ...
        self.ui.print_warning(
            f"Found {len(cancellable_batches)} batch(es) that can be cancelled"
        )
        ...
        self.ui.print_info(
            f"Processing cancellations for {len(cancellable_batches)} batch(es)..."
        )

        # Cancel batches
        cancelled_count, failed_count = self._cancel_batches(
            cancellable_batches, self.ui
        )
    ...
    def _cancel_batches(
        ...
        for tracking, status in batches:
            ...
            msg = (
                f"Cancelling batch {batch_id} ({provider}, status: '{status.value}')..."
            )
            if ui:
                ui.print_info(msg)
            else:
            ...
            try:
                ...
                if success:
                    ...
                    self.logger.warning(
                        f"Batch {batch_id} cancellation returned False."
                    )
                    if ui:
                        ui.display_batch_operation_result(
                            batch_id, "cancel", False, "Cancellation returned False"
                        )
                    else:
                        print(
